...
/**
 * Created by zhoutaotao on 3/24/15.
 */
@Controller
@RequestMapping("/data")
public class InitController {
    ...
    public String insert(@RequestParam("validate")String s){
        ...
        for (File product : productArray) {
            ...
            for (File sub : subFile) {
                if (sub.getName().contains(".png") || sub.getName().contains(".PNG")) {
            ...
            for (int i = 0; i < subFile.length; i++) {
                File sub = subFile[i];
                if (sub.getName().contains(".jpg") || sub.getName().contains(".JPG")) {
    ...
    @ResponseBody
    public String initInfo(@RequestParam("validate")String s){
        if(!s.equals("AEDIWLKESDKF")){
            return  "wrong";
        }
